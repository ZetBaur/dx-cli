<template>
    ...
    >
        ...
        <DxColumn data-field="OrderNumber" caption="Invoice Number" />
...
export default {
    ...
    methods: {
        getGroupCount(groupField) {
            return query(this.orders).groupBy(groupField).toArray().length;
        },
        groupChanged(e) {
            console.log(e);
            this.$refs[this.gridRefName].instance.clearGrouping();
            this.$refs[this.gridRefName].instance.columnOption(
                e.value,
                "groupIndex",
                0
            );
